export interface CommitteeChairAssignedEmailData {
  to: string;
  memberName: string;
...
export function committeeChairAssignedTemplate(
  data: CommitteeChairAssignedEmailData,
): { subject: string; html: string } {
  const year = new Date().getFullYear();
  const subject = `Appointed Chair — ${data.committeeName}`;
  const html = `<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"/><title>Committee Chair Appointment</title></head>
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border:1px solid #ddd8d0;border-radius:6px;overflow:hidden;">
        <tr><td style="background:#4B0082;padding:32px 48px 28px;">
          ...
          <h1 style="margin:0;font-size:20px;font-weight:normal;color:#fff;font-family:'Georgia',serif;">Committee Chair Appointment</h1>
        </td></tr>
        ...
        <tr><td style="padding:32px 48px 32px;">
          ...
          <p style="margin:0;font-size:15px;color:#2c2c2c;line-height:1.8;">
            You have been added as <strong>Chair</strong> of the <strong>${data.committeeName}</strong>.
          </p>
        </td></tr>
