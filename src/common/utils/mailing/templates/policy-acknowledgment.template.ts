export interface PolicyAcknowledgmentEmailData {
  to: string;
...
export function policyAcknowledgmentTemplate(
  data: PolicyAcknowledgmentEmailData,
): { subject: string; html: string } {
  const year = new Date().getFullYear();
  const subject = `Please acknowledge: ${data.policyTitle}`;
  const html = `<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"/><title>Policy Acknowledgment</title></head>
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border:1px solid #ddd8d0;border-radius:6px;overflow:hidden;">
        <tr><td style="background:#4B0082;padding:32px 48px 28px;">
          ...
          <h1 style="margin:0;font-size:20px;font-weight:normal;color:#fff;font-family:'Georgia',serif;">Board Policy Acknowledgment</h1>
        </td></tr>
        ...
        <tr><td style="padding:32px 48px 32px;">
          ...
          <p style="margin:0 0 20px;font-size:15px;color:#2c2c2c;line-height:1.8;">
            Please review and acknowledge <strong>${data.policyTitle}</strong>.
          </p>
          <div style="text-align:center;">
            <a href="${data.ackLink}" style="display:inline-block;background:#4B0082;color:#fff;text-decoration:none;padding:12px 28px;border-radius:6px;font-size:14px;">Review & Acknowledge</a>
          </div>
        </td></tr>
