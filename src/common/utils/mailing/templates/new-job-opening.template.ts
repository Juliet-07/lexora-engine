export interface NewJobOpeningEmailData {
  to: string;
...
export function newJobOpeningTemplate(data: NewJobOpeningEmailData): {
  subject: string;
  html: string;
} {
  const year = new Date().getFullYear();
  const subject = `New Opening: ${data.jobTitle}`;

  const html = `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"/><title>New Job Opening</title></head>
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0"
        ...
        <tr><td style="background:#4B0082;padding:32px 48px 28px;">
          ...
          <h1 style="margin:0;font-size:20px;font-weight:normal;color:#fff;font-family:'Georgia',serif;">New Job Opening</h1>
        </td></tr>
        ...
        <tr><td style="padding:32px 48px 32px;">
          ...
          <p style="margin:0 0 16px;font-size:15px;color:#2c2c2c;line-height:1.8;">
            A new position has just been posted at ${data.businessName}:
          </p>
          <p style="margin:0 0 24px;font-size:17px;color:#2c2c2c;font-weight:bold;">
            ${data.jobTitle}
          </p>
          <p style="margin:0;font-size:14px;color:#555;line-height:1.7;">
            If you know someone who'd be a great fit, feel free to refer them.
          </p>
        </td></tr>
