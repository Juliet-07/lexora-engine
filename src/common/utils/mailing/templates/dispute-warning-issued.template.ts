export interface DisputeWarningIssuedData {
  to: string;
...
export function disputeWarningIssuedTemplate(data: DisputeWarningIssuedData): {
  subject: string;
  html: string;
} {
  const firmName = process.env.FIRM_NAME || 'Lexora';
  const year = new Date().getFullYear();
  const subject = `${data.warningLevel} — Case ${data.caseNumber}`;

  const html = `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"/><title>${data.warningLevel}</title></head>
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0"
        ...
        <tr><td style="background:#4B0082;padding:32px 48px 28px;">
          ...
          <h1 style="margin:0;font-size:20px;font-weight:normal;color:#fff;font-family:'Georgia',serif;">${data.warningLevel}</h1>
        </td></tr>
        ...
        <tr><td style="padding:32px 48px 0;">
          ...
          <p style="margin:0 0 16px;font-size:15px;color:#2c2c2c;line-height:1.8;">
            Following the review of Case <strong>${data.caseNumber}</strong>, a ${data.warningLevel.toLowerCase()} has been issued to you.
          </p>
          ${
            data.notes
