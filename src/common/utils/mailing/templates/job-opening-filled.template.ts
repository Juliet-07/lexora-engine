export interface JobOpeningFilledEmailData {
  to: string;
...
export function jobOpeningFilledTemplate(data: JobOpeningFilledEmailData): {
  subject: string;
  html: string;
} {
  const year = new Date().getFullYear();
  const subject = `Position Filled: ${data.jobTitle}`;

  const html = `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"/><title>Position Filled</title></head>
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0"
        ...
        <tr><td style="background:#4B0082;padding:32px 48px 28px;">
          ...
          <h1 style="margin:0;font-size:20px;font-weight:normal;color:#fff;font-family:'Georgia',serif;">Position Filled</h1>
        </td></tr>
        ...
        <tr><td style="padding:32px 48px 32px;">
          ...
          <p style="margin:0;font-size:15px;color:#2c2c2c;line-height:1.8;">
            The <strong>${data.jobTitle}</strong> position at ${data.businessName} has now been filled. Thanks to everyone who referred candidates.
          </p>
        </td></tr>
