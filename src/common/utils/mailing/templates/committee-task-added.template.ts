export interface CommitteeTaskAddedEmailData {
  to: string[];
...
export function committeeTaskAddedTemplate(data: CommitteeTaskAddedEmailData): {
  subject: string;
  html: string;
} {
  const year = new Date().getFullYear();
  const subject = `New task — ${data.committeeName}`;
  const html = `<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"/><title>New Committee Task</title></head>
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border:1px solid #ddd8d0;border-radius:6px;overflow:hidden;">
        <tr><td style="background:#4B0082;padding:32px 48px 28px;">
          ...
          <h1 style="margin:0;font-size:20px;font-weight:normal;color:#fff;font-family:'Georgia',serif;">New Committee Task</h1>
        </td></tr>
        ...
        <tr><td style="padding:32px 48px 32px;">
          <p style="margin:0 0 16px;font-size:15px;color:#2c2c2c;line-height:1.8;">
            A new task has been added to the <strong>${data.committeeName}</strong>:
          </p>
          <p style="margin:0 0 12px;font-size:17px;color:#2c2c2c;font-weight:bold;">${data.taskTitle}</p>
          <p style="margin:0;font-size:14px;color:#555;line-height:1.7;">
            Owner: <strong>${data.owner}</strong> · Due ${data.dueDate.toLocaleDateString()}
          </p>
        </td></tr>
