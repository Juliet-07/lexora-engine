export interface DisputeRespondentReplyData {
  to: string;
...
export function disputeRespondentReplyTemplate(
  data: DisputeRespondentReplyData,
): { subject: string; html: string } {
  const firmName = process.env.FIRM_NAME || 'Lexora';
  const year = new Date().getFullYear();
  const subject = `A Response Has Been Submitted — Case ${data.caseNumber}`;

  const html = `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"/><title>Response Submitted</title></head>
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0"
        ...
        <tr><td style="background:#4B0082;padding:32px 48px 28px;">
          ...
          <h1 style="margin:0;font-size:20px;font-weight:normal;color:#fff;font-family:'Georgia',serif;">Response Submitted</h1>
        </td></tr>
        ...
        <tr><td style="padding:32px 48px 0;">
          ...
          <p style="margin:0 0 16px;font-size:15px;color:#2c2c2c;line-height:1.8;">
            ${data.respondentName} has submitted a response on Case <strong>${data.caseNumber}</strong>:
          </p>
