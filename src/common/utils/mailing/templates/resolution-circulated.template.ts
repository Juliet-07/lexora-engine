export interface ResolutionCirculatedEmailData {
  to: string;
  directorName: string;
  subject: string;
  reference: string;
...
export function resolutionCirculatedTemplate(
  data: ResolutionCirculatedEmailData,
): { subject: string; html: string } {
  const year = new Date().getFullYear();
  const subject = `${data.isReminder ? 'Reminder: ' : ''}Written Resolution ${data.reference} — Response Needed`;
  const html = `<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"/><title>Written Resolution</title></head>
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border:1px solid #ddd8d0;border-radius:6px;overflow:hidden;">
        <tr><td style="background:#4B0082;padding:32px 48px 28px;">
          ...
          <h1 style="margin:0;font-size:20px;font-weight:normal;color:#fff;font-family:'Georgia',serif;">${data.isReminder ? 'Reminder — ' : ''}Written Resolution</h1>
        </td></tr>
        <tr><td style="background:#c9a84c;height:3px;font-size:0;">&nbsp;</td></tr>
        <tr><td style="padding:32px 48px 32px;">
          <p style="margin:0 0 16px;font-size:15px;color:#2c2c2c;line-height:1.8;">Dear <strong>${data.directorName}</strong>,</p>
          <p style="margin:0 0 16px;font-size:15px;color:#2c2c2c;line-height:1.8;">
            Your response is requested on written resolution <strong>${data.reference}</strong>: <strong>${data.subject}</strong>.
          </p>
          <p style="margin:0;font-size:14px;color:#555;line-height:1.7;">
            Please respond by <strong>${data.deadline.toLocaleString()}</strong>.
          </p>
        </td></tr>
