export interface ComplianceDeadlineReminderEmailData {
  to: string;
  ownerName: string;
...
export function complianceDeadlineReminderTemplate(
  data: ComplianceDeadlineReminderEmailData,
): { subject: string; html: string } {
  const year = new Date().getFullYear();
  const subject = data.overdue
    ? `OVERDUE — ${data.obligationTitle}`
    : `${data.daysRemaining}-day reminder — ${data.obligationTitle}`;
  const headerColor = data.overdue ? '#b91c1c' : '#b45309';
  const html = `<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"/><title>Compliance Deadline</title></head>
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border:1px solid #ddd8d0;border-radius:6px;overflow:hidden;">
        <tr><td style="background:${headerColor};padding:32px 48px 28px;">
          ...
          <h1 style="margin:0;font-size:20px;font-weight:normal;color:#fff;font-family:'Georgia',serif;">
            ${data.overdue ? 'Obligation Overdue' : 'Deadline Reminder'}
          </h1>
        </td></tr>
        ...
        <tr><td style="padding:32px 48px 32px;">
          ...
          <p style="margin:0 0 16px;font-size:15px;color:#2c2c2c;line-height:1.8;">
            ${
              data.overdue
                ? `The following regulatory obligation is now <strong>overdue</strong>:`
                : `The following regulatory obligation is due in <strong>${data.daysRemaining} day${data.daysRemaining === 1 ? '' : 's'}</strong>:`
            }
          </p>
          <p style="margin:0 0 12px;font-size:17px;color:#2c2c2c;font-weight:bold;">${data.obligationTitle}</p>
          <p style="margin:0;font-size:14px;color:#555;line-height:1.7;">
            Regulator: <strong>${data.regulator}</strong> · Due ${data.dueDate.toLocaleDateString()}
          </p>
        </td></tr>
