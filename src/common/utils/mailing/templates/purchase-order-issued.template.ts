export interface PurchaseOrderIssuedEmailData {
  to: string;
  vendorName: string;
  ref: string;
  firmName: string;
...
// Deliberately brief — the real, formal document is the attached
// PDF, not this email body. This just tells the vendor it exists.
export function purchaseOrderIssuedTemplate(
  ...
  const subject = `Purchase Order ${data.ref} from ${data.firmName}`;
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0"
        ...
        <tr><td style="background:#4B0082;padding:32px 48px 28px;">
          ...
          <h1 style="margin:0;font-size:20px;font-weight:normal;color:#fff;font-family:'Georgia',serif;">Purchase Order ${data.ref}</h1>
        </td></tr>
        ...
        <tr><td style="padding:32px 48px 32px;">
          ...
          <p style="margin:0 0 16px;font-size:15px;color:#2c2c2c;line-height:1.8;">
            Please find attached purchase order <strong>${data.ref}</strong>, total <strong>${data.totalFormatted}</strong>.
          </p>
