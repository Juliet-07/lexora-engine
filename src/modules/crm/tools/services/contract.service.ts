...
export class ContractService {
  ...
  // Re-applies merge-field substitution using the contract's OWN
  // saved field values (scopeOfWork, jurisdiction, lead professional,
  // etc. — already persisted on the document at generation time),
  // against its current renderedBody.
  //
  // This exists for contracts generated before a merge field existed
  // in the code: renderContractBody() deliberately leaves an unmapped
  // {{token}} literal rather than blanking it (see contract-fields.util),
  // so a contract generated by an older build still shows the raw
  // {{tenantCompanyJurisdiction}}-style token today even though the
  // field is now supported and the tenant already filled it in on the
  // drafting form — the stored renderedBody was simply never re-rendered.
  // Re-running renderContractBody() against the CURRENT renderedBody
  // (not the original template) is safe and idempotent: any field
  // already substituted the first time round is now plain text, no
  // longer wrapped in {{}}, so this only ever fills in what's still
  // outstanding — it can't double-substitute or corrupt prior edits.
  // Same edit window as editRenderedBody: Not Sent or Sent only.
  async resyncMergeFields(
    tenantId: string,
    contractId: string,
  ): Promise<ToolContractDocument_ & { unresolvedMergeFields?: string[] }> {
    const contract = await this.model.findOne({
      _id: contractId,
      tenantId: new Types.ObjectId(tenantId),
    });
    ...
    const businessName = await resolveBusinessName(this.userModel, tenantId);
    const createdAt = (contract as any).createdAt as Date | undefined;
    const fields: Record<ContractMergeField, string> = {
      title: contract.title,
      counterpartyName: contract.counterparty,
      recipientName: contract.counterparty,
      recipientEmail: contract.counterpartyEmail,
      scopeOfWork: formatScopeOfWorkList(contract.scopeOfWork ?? ''),
      tenantCompanyName: businessName,
      contractValue: contract.value != null ? String(contract.value) : '',
      contractCurrency: contract.currency ?? 'USD',
      effectiveDate: createdAt
        ? new Date(createdAt).toISOString().slice(0, 10)
        : '',
      expiryDate: contract.expiresOn
        ? new Date(contract.expiresOn).toISOString().slice(0, 10)
        : '',
      todayDate: new Date().toISOString().slice(0, 10),
      tenantCompanyJurisdiction: contract.tenantCompanyJurisdiction ?? '',
      clientJurisdiction: contract.clientJurisdiction ?? '',
      leadProfessionalName: contract.leadProfessionalName ?? '',
      leadProfessionalTitle: contract.leadProfessionalTitle ?? '',
      clientRepresentativeName: contract.clientRepresentativeName ?? '',
      clientRepresentativeTitle: contract.clientRepresentativeTitle ?? '',
      commencementDate: contract.commencementDate
        ? new Date(contract.commencementDate).toISOString().slice(0, 10)
        : '',
      engagementDuration: contract.engagementDuration ?? '',
      tenantRegisteredAddress: contract.tenantRegisteredAddress ?? '',
      clientRegisteredAddress: contract.clientRegisteredAddress ?? '',
      serviceCategory: contract.serviceCategory ?? '',
    };

    contract.renderedBody = renderContractBody(contract.renderedBody, fields);
    contract.interactions.push({
      type: ToolContractInteractionType.UPDATED,
      occurredAt: new Date(),
      actor: 'tenant',
      message: "Merge fields resynced from the contract's own saved details.",
      tenantUserId: null,
    } as any);
    await contract.save();

    const unresolvedMergeFields = findUnresolvedTokens(contract.renderedBody);
    return Object.assign(contract, { unresolvedMergeFields });
  }
