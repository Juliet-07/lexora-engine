import { Module } from '@nestjs/common';
import { MongooseModule } from '@nestjs/mongoose';
import { ProjectsModule } from 'src/modules/crm/projects/project.module';
import { HrModule } from 'src/modules/hr/hr.module';
import { EmailService } from 'src/common/utils/mailing/email.service';
import { User, UserSchema } from 'src/modules/auth/schemas/user.schema';
import {
  Vendor,
  VendorSchema,
} from 'src/modules/crm/crm/schemas/vendor.schema';
import {
  CalendarEvent,
  CalendarEventSchema,
} from 'src/modules/crm/tools/schemas/calendar.schema';
import {
  Invoice,
  InvoiceSchema,
  RemittanceAccount,
  RemittanceAccountSchema,
  Payment,
  PaymentSchema,
  CreditNote,
  CreditNoteSchema,
  WriteOff,
  WriteOffSchema,
  Quote,
  QuoteSchema,
  RecurringInvoice,
  RecurringInvoiceSchema,
  PaymentPlan,
  PaymentPlanSchema,
  PurchaseOrder,
  PurchaseOrderSchema,
  Bill,
  BillSchema,
  ExpenseClaim,
  ExpenseClaimSchema,
  ExpensePolicy,
  ExpensePolicySchema,
  BankAccount,
  BankAccountSchema,
  BankTransaction,
  BankTransactionSchema,
  BankRule,
  BankRuleSchema,
  Transfer,
  TransferSchema,
  Reconciliation,
  ReconciliationSchema,
  TaxObligation,
  TaxObligationSchema,
  WhtCertificate,
  WhtCertificateSchema,
  LedgerAccount,
  LedgerAccountSchema,
  GlEntry,
  GlEntrySchema,
  Journal,
  JournalSchema,
  AccountingPeriod,
  AccountingPeriodSchema,
  Asset,
  AssetSchema,
  MaintenanceLogEntry,
  MaintenanceLogEntrySchema,
  TrustLedger,
  TrustLedgerSchema,
  TrustMovement,
  TrustMovementSchema,
  Fund,
  FundSchema,
  CapitalCommitment,
  CapitalCommitmentSchema,
  CapitalCall,
  CapitalCallSchema,
  CapitalAccountEntry,
  CapitalAccountEntrySchema,
  Distribution,
  DistributionSchema,
  PortfolioHolding,
  PortfolioHoldingSchema,
  HoldingValuation,
  HoldingValuationSchema,
  FundExpense,
  FundExpenseSchema,
  ManagementFeeCharge,
  ManagementFeeChargeSchema,
  KeyPerson,
  KeyPersonSchema,
  ComplianceCalendarItem,
  ComplianceCalendarItemSchema,
  FxRate,
  FxRateSchema,
  Budget,
  BudgetSchema,
  ManagementReport,
  ManagementReportSchema,
} from './schemas';
import {
  WriteOffService,
  InvoiceService,
  ClientInvoiceService,
  RemittanceAccountService,
  WipService,
  PaymentService,
  CreditNoteService,
  QuoteService,
  RecurringInvoiceService,
  PaymentPlanService,
  InstalmentAutoSendService,
  VendorService,
  PurchaseOrderService,
  BillService,
  ExpenseClaimService,
  ExpensePolicyService,
  PurchasesOverviewService,
  BillPaymentReminderService,
  BankAccountService,
  BankTransactionService,
  BankRuleService,
  TransferService,
  ReconciliationService,
  CashForecastService,
  WhtService,
  VatService,
  PayrollTaxService,
  CitService,
  EbmService,
  TaxObligationService,
  TaxObligationReminderService,
  GlPostingService,
  LedgerAccountService,
  JournalService,
  RecodeService,
  GeneralLedgerService,
  TrialBalanceService,
  PeriodCloseService,
  AssetService,
  MaintenanceLogService,
  AccountingOverviewService,
  FinancialStatementsService,
  TrustLedgerService,
  TrustMovementService,
  FundService,
  CapitalCommitmentService,
  CapitalCallService,
  CapitalAccountService,
  DistributionService,
  PortfolioHoldingService,
  HoldingValuationService,
  NavService,
  FundExpenseService,
  ManagementFeeService,
  ComplianceService,
  FxRateService,
  ScenarioService,
  LpReportingService,
  BudgetService,
  ManagementReportingService,
} from './services';
import {
  WriteOffController,
  WipController,
  InvoiceController,
  ClientInvoiceController,
  RemittanceAccountController,
  PaymentController,
  CreditNoteController,
  QuoteController,
  RecurringInvoiceController,
  PaymentPlanController,
  VendorController,
  PurchaseOrderController,
  BillController,
  ExpenseClaimController,
  ExpensePolicyController,
  PurchasesOverviewController,
  BankAccountController,
  BankTransactionController,
  BankRuleController,
  TransferController,
  ReconciliationController,
  CashForecastController,
  WhtController,
  VatController,
  PayrollTaxController,
  CitController,
  EbmController,
  TaxObligationController,
} from './controllers';
import {
  AccountingOverviewController,
  LedgerAccountController,
  JournalController,
  GeneralLedgerController,
  TrialBalanceController,
  PeriodCloseController,
  RecodeController,
  AssetController,
  MaintenanceLogController,
  FinancialStatementsController,
  TrustLedgerController,
  TrustMovementController,
  FundController,
  CapitalCommitmentController,
  CapitalCallController,
  CapitalAccountController,
  DistributionController,
  PortfolioHoldingController,
  HoldingValuationController,
  NavController,
  FundExpenseController,
  ManagementFeeController,
  ComplianceController,
  FxRateController,
  ScenarioController,
  LpReportingController,
  BudgetController,
  ManagementReportingController,
} from './controllers';

@Module({
  imports: [
    ProjectsModule,
    HrModule,
    MongooseModule.forFeature([
      { name: Invoice.name, schema: InvoiceSchema },
      { name: RemittanceAccount.name, schema: RemittanceAccountSchema },
      { name: Payment.name, schema: PaymentSchema },
      { name: CalendarEvent.name, schema: CalendarEventSchema },
      { name: ManagementReport.name, schema: ManagementReportSchema },
      { name: CreditNote.name, schema: CreditNoteSchema },
      { name: WriteOff.name, schema: WriteOffSchema },
      { name: Quote.name, schema: QuoteSchema },
      { name: RecurringInvoice.name, schema: RecurringInvoiceSchema },
      { name: PaymentPlan.name, schema: PaymentPlanSchema },
      { name: Vendor.name, schema: VendorSchema },
      { name: PurchaseOrder.name, schema: PurchaseOrderSchema },
      { name: Bill.name, schema: BillSchema },
      { name: ExpenseClaim.name, schema: ExpenseClaimSchema },
      { name: ExpensePolicy.name, schema: ExpensePolicySchema },
      { name: BankAccount.name, schema: BankAccountSchema },
      { name: BankTransaction.name, schema: BankTransactionSchema },
      { name: BankRule.name, schema: BankRuleSchema },
      { name: Transfer.name, schema: TransferSchema },
      { name: Reconciliation.name, schema: ReconciliationSchema },
      { name: TaxObligation.name, schema: TaxObligationSchema },
      { name: WhtCertificate.name, schema: WhtCertificateSchema },
      { name: LedgerAccount.name, schema: LedgerAccountSchema },
      { name: GlEntry.name, schema: GlEntrySchema },
      { name: Journal.name, schema: JournalSchema },
      { name: AccountingPeriod.name, schema: AccountingPeriodSchema },
      { name: Asset.name, schema: AssetSchema },
      { name: MaintenanceLogEntry.name, schema: MaintenanceLogEntrySchema },
      { name: TrustLedger.name, schema: TrustLedgerSchema },
      { name: TrustMovement.name, schema: TrustMovementSchema },
      { name: Fund.name, schema: FundSchema },
      { name: CapitalCommitment.name, schema: CapitalCommitmentSchema },
      { name: CapitalCall.name, schema: CapitalCallSchema },
      { name: CapitalAccountEntry.name, schema: CapitalAccountEntrySchema },
      { name: Distribution.name, schema: DistributionSchema },
      { name: PortfolioHolding.name, schema: PortfolioHoldingSchema },
      { name: HoldingValuation.name, schema: HoldingValuationSchema },
      { name: FundExpense.name, schema: FundExpenseSchema },
      { name: ManagementFeeCharge.name, schema: ManagementFeeChargeSchema },
      { name: KeyPerson.name, schema: KeyPersonSchema },
      {
        name: ComplianceCalendarItem.name,
        schema: ComplianceCalendarItemSchema,
      },
      { name: FxRate.name, schema: FxRateSchema },
      { name: Budget.name, schema: BudgetSchema },
      { name: User.name, schema: UserSchema },
    ]),
  ],
  providers: [
    WriteOffService,
    InvoiceService,
    ClientInvoiceService,
    RemittanceAccountService,
    WipService,
    PaymentService,
    CreditNoteService,
    QuoteService,
    RecurringInvoiceService,
    PaymentPlanService,
    InstalmentAutoSendService,
    VendorService,
    PurchaseOrderService,
    BillService,
    ExpenseClaimService,
    ExpensePolicyService,
    PurchasesOverviewService,
    BillPaymentReminderService,
    BankAccountService,
    BankTransactionService,
    BankRuleService,
    TransferService,
    ReconciliationService,
    CashForecastService,
    WhtService,
    VatService,
    PayrollTaxService,
    CitService,
    EbmService,
    TaxObligationService,
    TaxObligationReminderService,
    GlPostingService,
    LedgerAccountService,
    JournalService,
    RecodeService,
    GeneralLedgerService,
    TrialBalanceService,
    PeriodCloseService,
    AssetService,
    MaintenanceLogService,
    AccountingOverviewService,
    FinancialStatementsService,
    TrustLedgerService,
    TrustMovementService,
    FundService,
    CapitalCommitmentService,
    CapitalCallService,
    CapitalAccountService,
    DistributionService,
    PortfolioHoldingService,
    HoldingValuationService,
    NavService,
    FundExpenseService,
    ManagementFeeService,
    ComplianceService,
    FxRateService,
    ScenarioService,
    LpReportingService,
    BudgetService,
    ManagementReportingService,
    EmailService,
  ],
  controllers: [
    WriteOffController,
    WipController,
    InvoiceController,
    ClientInvoiceController,
    RemittanceAccountController,
    PaymentController,
    CreditNoteController,
    QuoteController,
    RecurringInvoiceController,
    PaymentPlanController,
    VendorController,
    PurchaseOrderController,
    BillController,
    ExpenseClaimController,
    ExpensePolicyController,
    PurchasesOverviewController,
    BankAccountController,
    BankTransactionController,
    BankRuleController,
    TransferController,
    ReconciliationController,
    CashForecastController,
    WhtController,
    VatController,
    PayrollTaxController,
    CitController,
    EbmController,
    TaxObligationController,
    AccountingOverviewController,
    LedgerAccountController,
    JournalController,
    GeneralLedgerController,
    TrialBalanceController,
    PeriodCloseController,
    RecodeController,
    AssetController,
    MaintenanceLogController,
    FinancialStatementsController,
    TrustLedgerController,
    TrustMovementController,
    FundController,
    CapitalCommitmentController,
    CapitalCallController,
    CapitalAccountController,
    DistributionController,
    PortfolioHoldingController,
    HoldingValuationController,
    NavController,
    FundExpenseController,
    ManagementFeeController,
    ComplianceController,
    FxRateController,
    ScenarioController,
    LpReportingController,
    BudgetController,
    ManagementReportingController,
  ],
  exports: [InvoiceService, WriteOffService],
})
export class FinanceModule {}
